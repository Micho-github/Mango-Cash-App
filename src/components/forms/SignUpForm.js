...
export default function SignUpForm() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('http://localhost:8800/Api/V1/Account/SignUp', {
                phoneNumber,
                username,
                email,
                password,
        ...
        <div className="form-block w-form">
            <form onSubmit={handleSubmit} data-name="Get In Touch Form" name="wf-form-Get-In-Touch-Form" id="email-form" method="get"
                ...
                data-wf-element-id="8852d4f2-6216-f765-8ab0-654bda743318">
                    ...
                    placeholder="Enter your email" type="tel" id="phone-2" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} required="" />
                    <label for="username"
                    ...
                    required="" />
                    <label for="email-5" className="field-label">Email address</label>
                    <input className="text-field w-input" maxlength="256" name="email" data-name="email"
                    placeholder="Enter your email" type="email" id="email-5" value={email} onChange={(e) => setEmail(e.target.value)} required="" />
                    <label for="password"
